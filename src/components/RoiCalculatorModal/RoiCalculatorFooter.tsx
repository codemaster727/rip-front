import { Box, ExpandableLabel, Flex, Grid, HelpIcon, LinkExternal, Text, useTooltip } from "@pancakeswap/uikit";
...
const RoiCalculatorFooter: React.FC<RoiCalculatorFooterProps> = ({
  ...
  return (
    <Footer mt="1rem" p="16px" flexDirection="column">
        ...
        <Box px="8px">
          <Grid gridTemplateColumns="2.5fr 1fr" gridRowGap="8px" gridTemplateRows={`repeat(${gridRowCount}, auto)`}>
            ...
            {!Number.isFinite(apy) && (
              <Text small textAlign="right">
                {(
                  getApy(apr as number, autoCompoundFrequency > 0 ? autoCompoundFrequency : 1, 365, performanceFee) *
                  100
                ).toFixed(2)}
                %
              </Text>
