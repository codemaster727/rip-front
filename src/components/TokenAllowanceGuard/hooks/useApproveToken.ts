...
export const useApproveToken = (tokenAddressMap: AddressMap, spenderAddressMap: AddressMap) => {
  ...
  return useMutation<ContractReceipt, Error>(
    async () => {
      const contractAddress = spenderAddressMap[chainId as keyof typeof spenderAddressMap];
    ...
    },
    {
      ...
      onSuccess: async () => {
        ...
        await client.refetchQueries(contractAllowanceQueryKey(address, chainId, tokenAddressMap, spenderAddressMap));
      },
    },
  );
};
