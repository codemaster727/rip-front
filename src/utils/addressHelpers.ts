...
import { VaultKey } from "../slices/types";

export const getAddress = (address: Address): string => {
  const chainId: string = process.env.REACT_APP_PUBLIC_CHAIN_ID ?? "56";
  return (
    address[parseInt(chainId) as keyof Address]
      ? address[parseInt(chainId) as keyof Address]
      : address[ChainId.MAINNET as keyof Address]
  ) as string;
};
