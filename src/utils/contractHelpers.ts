...
import { PoolCategory, SerializedPoolConfig } from "../constants/types";
// Addresses
import {
  getAddress,
...
export const getSouschefContract = (id: number, signer?: Signer | Provider) => {
  ...
  const abi = (config as SerializedPoolConfig).poolCategory === PoolCategory.BINANCE ? sousChefBnb : sousChef;
  return getContract(abi, getAddress((config as SerializedPoolConfig).contractAddress), signer) as SousChef;
};
export const getSouschefV2Contract = (id: number, signer?: Signer | Provider) => {
  const config = poolsConfig.find(pool => pool.sousId === id);
  return getContract(sousChefV2, getAddress((config as SerializedPoolConfig).contractAddress), signer) as SousChefV2;
};
