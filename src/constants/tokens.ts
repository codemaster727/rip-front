...
import { serializeToken } from "../slices/user/hooks/helpers";
...
const tokens = () => {
  ...
  if (parseInt(chainId ?? "56", 10) === ChainId.TESTNET) {
    return Object.keys(mainnetTokens).reduce((accum, key) => {
      return {
        ...accum,
        [key]: testnetTokens[key as keyof typeof testnetTokens] || mainnetTokens[key as keyof typeof mainnetTokens],
      };
...
export const serializeTokens = () => {
  const serializedTokens = Object.keys(unserializedTokens).reduce((accum, key) => {
    return { ...accum, [key]: serializeToken(unserializedTokens[key as keyof typeof unserializedTokens]) };
