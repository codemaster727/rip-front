...
export function useDerivedSwapInfo(
  ...
  const relevantTokenBalances = useCurrencyBalances(account ? (account !== "" ? account : undefined) : undefined, [
...
export const useFetchPairPrices = ({
  ...
  const pairData: any = useSelector(pairByDataIdSelector({ pairId, timeWindow }));
  const derivedPairData: any = useSelector(derivedPairByDataIdSelector({ pairId, timeWindow }));
  ...
  let pairPrices: PairPricesNormalized | undefined =
    hasNoDirectData && hasNoDerivedData ? ([] as PairPricesNormalized) : undefined;
