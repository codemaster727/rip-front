...
import { ONE_DAY_UNIX, PCS_V2_START } from "src/constants/info";
import { TransactionType } from "src/slices/info/types";
...
export const fetchChartData = async (
  ...
  while (!allFound) {
    ...
    const { data, error: fetchError } = await getEntityDayDatas(skip, address as string);
    skip += 1000;
    allFound = (data as ChartEntry[]).length < 1000;
    error = fetchError;
    if (data) {
