...
import { DeserializedFarm, DeserializedFarmUserData, SerializedFarm, State } from "../types";
...
const selectFarmByKey = (key: string, value: string | number) => (state: State) =>
  state.farms.data.find(f => f[key as keyof typeof f] === value);

export const makeFarmFromPidSelector = (pid: number) =>
  createSelector([selectFarmByKey("pid", pid)], farm => deserializeFarm(farm as SerializedFarm));

export const makeBusdPriceFromPidSelector = (pid: number) =>
  createSelector([selectFarmByKey("pid", pid)], farm => {
    const deserializedFarm = deserializeFarm(farm as SerializedFarm);
    return deserializedFarm && new BigNumber(deserializedFarm.tokenPriceBusd as string);
  });

export const makeUserFarmFromPidSelector = (pid: number) =>
  createSelector([selectFarmByKey("pid", pid)], farm => {
    const { userData } = deserializeFarm(farm as SerializedFarm);
    const { allowance, tokenBalance, stakedBalance, earnings } = userData as DeserializedFarmUserData;
    return {
      allowance,
      tokenBalance,
      stakedBalance,
...
export const priceCakeFromPidSelector = createSelector([selectCakeFarm], cakeBnbFarm => {
  const deserializedCakeBnbFarm = deserializeFarm(cakeBnbFarm as SerializedFarm);
...
export const farmFromLpSymbolSelector = (lpSymbol: string) =>
  createSelector([selectFarmByKey("lpSymbol", lpSymbol)], farm => deserializeFarm(farm as SerializedFarm));

export const makeLpTokenPriceFromLpSymbolSelector = (lpSymbol: string) =>
  createSelector([selectFarmByKey("lpSymbol", lpSymbol)], farm => {
    const deserializedFarm = deserializeFarm(farm as SerializedFarm);
    const farmTokenPriceInUsd = deserializedFarm && new BigNumber(deserializedFarm.tokenPriceBusd as string);
    let lpTokenPrice = BIG_ZERO;

    if (deserializedFarm?.lpTotalSupply?.gt(0) && deserializedFarm?.lpTotalInQuoteToken?.gt(0)) {
      // Total value of base token in LP
      const valueOfBaseTokenInFarm = farmTokenPriceInUsd.times(deserializedFarm.tokenAmountTotal as BigNumber);
