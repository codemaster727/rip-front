...
import { DEFAULT_LIST_OF_LISTS, OFFICIAL_LISTS } from "../../constants/lists";
...
import { EMPTY_LIST, TagInfo, TokenAddressMap, WrappedTokenInfo } from "../types";
...
export const combinedTokenMapFromActiveUrlsSelector = createSelector(
  ...
  (lists, urls) => {
    return combineTokenMapsWithDefault(lists, urls as string[]);
  },
);
    ...
    const tags: TagInfo[] =
      tokenInfo.tags
        ?.map((value: string, index: number) => {
          ...
          return { ...list.tags![value], id: value };
        })
