...
import { AutoColumn } from "src/components/Layout/Column";
import Footer from "src/components/Menu/Footer";
import { GELATO_NATIVE, LIMIT_ORDERS_DOCS_URL } from "src/constants";
import { useTranslation } from "src/contexts/Localization";
// import useActiveWeb3React from "src/hooks/useActiveWeb3React";
import { useWeb3Context } from "src/hooks";
import useGasOverhead from "src/hooks/limitOrders/useGasOverhead";
import useGelatoLimitOrders from "src/hooks/limitOrders/useGelatoLimitOrders";
import { ApprovalState, useApproveCallbackFromInputCurrencyAmount } from "src/hooks/useApproveCallback";
import useTheme from "src/hooks/useTheme";
import { useDefaultsFromURLSearch } from "src/slices/limitOrders/hooks";
import { Field } from "src/slices/limitOrders/types";
import { useExchangeChartManager } from "src/slices/user/hooks";
import { maxAmountSpend } from "src/utils/maxAmountSpend";
import PriceChartContainer from "src/views/Swap/components/Chart/PriceChartContainer";
...
const LimitOrders = () => {
  ...
  const percentageRateDifference = getRatePercentageDifference(currentMarketRate as Price, price as Price);
  ...
  const [showConfirmModal] = useModal(
    <ConfirmLimitOrderModal
      currencies={currencies as { input: Currency | Token; output: Currency | Token }}
      formattedAmounts={formattedAmounts}
      currentMarketRate={currentMarketRate?.toSignificant(4) as string}
      currentMarketRateInverted={currentMarketRate?.invert().toSignificant(4) as string}
      limitPrice={price?.toSignificant(6) as string}
      limitPriceInverted={price?.invert().toSignificant(6) as string}
      percentageRateDifference={parseFloat(percentageRateDifference?.toSignificant(3) as string).toLocaleString(
        undefined,
        {
      ...
      swapErrorMessage={swapErrorMessage as string}
    />,
    true,
    true,
    "confirmLimitOrderModal",
    ...
    >
      ...
      >
        {!isMobile && (
          <Flex width={isChartExpanded ? "100%" : "50%"} flexDirection="column">
            ...
            <Box width="100%">{/* <LimitOrderTable isCompact={isTablet} /> */}</Box>
          </Flex>
        )}
                ...
                <Wrapper id="limit-order-page" style={{ minHeight: "412px" }}>
                  <AutoColumn gap="sm">
                    ...
                    <SwitchTokensButton
                      ...
                      color={
                        currencies[Field.INPUT.toLowerCase() as keyof typeof currencies] &&
                        currencies[Field.OUTPUT.toLowerCase() as keyof typeof currencies]
                          ? "primary"
                          : "text"
                      }
                    />
                    ...
                    <LimitOrderPrice
                      ...
                      realExecutionPriceAsString={!inputError ? (realExecutionPriceAsString as any) : ""}
                  ...
                  <Box mt="0.25rem">
                    {!account ? (
                      ...
                      >
                        {approvalSubmitted
                          ? t("Enabling %asset%", { asset: currencies.input?.symbol as string })
                          : t("Enable %asset%", { asset: currencies.input?.symbol as string })}
                      </Button>
                    ) : (
                      <Button
                        variant="primary"
                  ...
                  <Flex mt="16px" justifyContent="center">
                    <Link external href="https://www.gelato.network/">
                      <img
                        src={
                          theme === "dark"
                            ? "/images/powered_by_gelato_white.svg"
                            : "/images/powered_by_gelato_black.svg"
                        }
      ...
      <BottomDrawer
        content={
          <PriceChartContainer
            inputCurrencyId={currencyIds.input}
            inputCurrency={currencies[Field.INPUT.toLowerCase() as keyof typeof currencies] as Currency | Token}
            outputCurrencyId={currencyIds.output}
            outputCurrency={currencies[Field.OUTPUT.toLowerCase() as keyof typeof currencies] as Currency | Token}
            isChartExpanded={isChartExpanded}
            setIsChartExpanded={setIsChartExpanded}
            isChartDisplayed={isChartDisplayed}
            currentSwapPrice={singleTokenPrice}
