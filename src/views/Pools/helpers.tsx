...
import { vaultPoolConfig } from "src/constants/pools";
import { DeserializedPool } from "src/slices/types";
import { BIG_ZERO } from "src/utils/bigNumber";
import { getApy } from "src/utils/compoundApyHelpers";
import { getBalanceNumber, getDecimalAmount, getFullDisplayBalance } from "src/utils/formatBalance";
...
export const getAprData = (pool: DeserializedPool, performanceFee: number) => {
  ...
  if (vaultKey) {
    const autoApr = getApy(apr as number, autoCompoundFrequency, 365, performanceFee) * 100;
