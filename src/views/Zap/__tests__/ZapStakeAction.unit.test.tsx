...
import * as Balances from "src/hooks/useBalances";
...
describe("<ZapStakeAction/> ", () => {
  beforeEach(() => {
    ...
    //@ts-expect-error
    Balances.useSripBalance = jest.fn().mockReturnValue({ 1: { data: new DecimalBigNumber("10") } });
    //@ts-expect-error
    Balances.useGripBalance = jest.fn().mockReturnValue({ 1: { data: new DecimalBigNumber("10") } });
    //@ts-expect-error
    Prices.useGripPrice = jest.fn().mockReturnValue({ data: "3400.00" });
    //@ts-expect-error
    Prices.useRipPrice = jest.fn().mockReturnValue({ data: "32.00" });
    //@ts-expect-error
    ContractAllowance.useContractAllowance = jest.fn().mockReturnValue({ data: BigNumber.from(0) });
  });

  it("gRIP should autopopulate with correct value based on ETH input", async () => {
    render(
      <>
        <ZapStakeAction />
      </>,
    ...
    fireEvent.click(await screen.getAllByText("gRIP")[0]);
  ...
  it("sRIP should autopopulate with correct value based on ETH input", async () => {
    //@ts-expect-error
    ContractAllowance.useContractAllowance = jest.fn().mockReturnValue({ data: BigNumber.from(10) });
    render(
      <>
    ...
    expect(await screen.findByText("Enter Amount"));
    fireEvent.click(await screen.findByTestId("zap-output"));
    fireEvent.click(await screen.getAllByText("sRIP")[0]);
