...
import { useUserSingleHopOnly } from "../slices/user/hooks";
...
export function useAllCommonPairs(currencyA?: Currency, currencyB?: Currency): Pair[] {
  ...
  const bases: Token[] = useMemo(() => {
    ...
    const additionalA = tokenA
      ? ADDITIONAL_BASES[chainId as keyof typeof ADDITIONAL_BASES]?.[tokenA.address] ?? []
      : [];
    const additionalB = tokenB
      ? ADDITIONAL_BASES[chainId as keyof typeof ADDITIONAL_BASES]?.[tokenB.address] ?? []
      : [];
