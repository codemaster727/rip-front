...
} from "../slices/lists/hooks";
import { NEVER_RELOAD, useSingleCallResult } from "../slices/multicall/hooks";
import { TokenAddressMap } from "../slices/types";
import useUserAddedTokens, { userAddedTokenSelector } from "../slices/user/hooks/useUserAddedTokens";
import { isAddress } from "../utils";
import { useBytes32TokenContract, useTokenContract } from "./useContract";

const mapWithoutUrls = (tokenMap: TokenAddressMap) =>
  Object.keys(tokenMap[parseInt(CHAIN_ID === undefined ? "56" : CHAIN_ID) as keyof typeof tokenMap]).reduce<{
    [address: string]: Token;
  }>((newMap, address) => {
    newMap[address] =
      tokenMap[parseInt(CHAIN_ID === undefined ? "56" : CHAIN_ID) as keyof typeof tokenMap][address].token;
    return newMap;
  }, {});
